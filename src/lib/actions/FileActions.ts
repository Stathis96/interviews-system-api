...
export async function uploadFileAction (id: string, file: string, connection: Knex): Promise<PdfFile> {
  ...
  await fsPromise.writeFile(`${filepath}/${interview.interviewId}${interview.bio.length > 0 ? '-' + interview.bio.length.toString() : ''}.pdf`, base64File, { encoding: 'base64' }).catch(e => console.log(e))
